...
FROM
    ESTOQUE E
    JOIN COLONIA C
        ON E.TURNO = :TURNO_ATUAL
        AND E.PLANETA = C.PLANETA
        AND C.IMPERIO = :IMPERIO AND C.TURNO_INICIAL <= :TURNO_ATUAL AND (C.TURNO_FINAL >= :TURNO_ATUAL OR C.TURNO_FINAL IS NULL)
